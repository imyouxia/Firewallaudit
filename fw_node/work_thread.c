		...
		for (i = 0; i < fds; i++) {
			...
			if (self->tq->pipes[0] == ev_list[i].data.fd) {
				...
				while(tasks > 0) {
					...
					if (new_task->execute == SERIAL) {
						add_task_without_notify((workers + num - 1)->tq, new_task);
					} else {
...
static void * worker_thread_func(void * client_data)
{
	...
	while(self->stop != 1) {
		...
		tasks = tq_tasks_num(self->tq);
		while(tasks > 0) {
zlog_error(logger, "BEFORE\t\t\t[TASK NUM]%d", tasks);
			pop_task(self->tq, &tn);
			if (tn == NULL) {
zlog_error(logger, "\t\t\t[TASK IS NULL]");
				break;
			}

			if (tn->func) {
				tn->func(tn->client_data);
zlog_error(logger, "\t\t\t[EXECUTE Function]");
			} else {
zlog_error(logger, "///////////////////////////// Function is NULL");
			}
			delete_task_node(&tn);
			tasks--;
zlog_error(logger, "AFTER\t\t\t[TASK NUM]%d", tasks);
		}
	}
}
