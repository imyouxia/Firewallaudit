				...
				const char * prompt, const char * localpath)
{
	...
	strcpy(cmdbuff, localpath);
	char * temp = strrchr(cmdbuff, '/');
	if (temp != NULL) *temp = '\0';
...
int main(int argc, char * argv[])
{
	...
	ssh2_dst_info info;
	info.hostname = "10.109.32.152";
	info.username = "superman";
	info.password = "talent";
	info.port = 22;
...
int ret = cmd_exec_via_pseudoterm(conn, "system telnetd start", "TopsecOS#", "localfile");
//system telentd stop
